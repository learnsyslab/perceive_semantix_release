...
from perceive_semantix_ros2.ros2_pointcloud import array_to_pointcloud2, merge_rgb_fields
...
class PerceiveSemantixNode(Node):
    def __init__(self) -> None:
        ...
        # fmt: off
        self.declare_parameter("topic_camera_info"                      , "spectacular_ai/camera_info"  , ParameterDescriptor(read_only=True, type=ParameterType.PARAMETER_STRING))
        self.declare_parameter("topic_color"                            , "spectacular_ai/color_image"  , ParameterDescriptor(read_only=True, type=ParameterType.PARAMETER_STRING))
        self.declare_parameter("topic_depth"                            , "spectacular_ai/depth_image"  , ParameterDescriptor(read_only=True, type=ParameterType.PARAMETER_STRING))
        ...
        self.declare_parameter("publishing_rate_background_pointcloud"  , -1.0                          , ParameterDescriptor(read_only=True, type=ParameterType.PARAMETER_DOUBLE, description="Rate (in Hz) at which the background point cloud is published. Set to -1 to publish every time the scene is updated. Set to 0 to disable publishing."))
        ...
        self.publishing_rate_background_pointcloud: float   = float(self.get_parameter("publishing_rate_background_pointcloud").get_parameter_value().double_value)
        ...
        verify_publishing_rate("publishing_rate_background_pointcloud", self.publishing_rate_background_pointcloud)
        ...
        self.callback_synchronizer.registerCallback(self._callback_sync)
        self.input: Optional[tuple[InputDataStamped, str, RosTime]] = None
        ...
        debug_config_args: dict[str, Any] = {"store_scene_each_frame": store_output}
        ...
        if self.publishing_rate_background_pointcloud != 0:
            self.publisher_background = self.create_publisher(PointCloud2, "map/background", 1)
        else:
    ...
    def _callback_sync(self, info_msg: CameraInfo, color_msg: ROSImage, depth_msg: ROSImage) -> None:
        data = InputData(
    ...
    def _get_pose(self, camera_frame: str, time: Optional[RosTime] = None) -> Optional[Float[np.ndarray, "4 4"]]:
        if time is None:
            time = self.get_clock().now()
        try:
            transform_msg = self.tf_buffer.lookup_transform(self.global_frame, camera_frame, time)
            return self._process_pose(transform_msg)
        except ExtrapolationException as e:
            self.get_logger().warning(f"Transform not ready: {e}", once=True)
            return None
        except Exception as e:
            self.get_logger().error(f"Exception when looking up transform: {e}", throttle_duration_sec=1.0)
            return None
    ...
    def _try_complete_input(self) -> bool:
        if self.input is None or self.input[0].data is None:
            return False
        camera_frame = self.input[1]
        input_time = self.input[2]
        if (camera_pose := self._get_pose(camera_frame, input_time)) is None:
            return False
        self.input[0].data.pose = camera_pose
        return True
    ...
    def _publish_background_pointcloud(self, background: GeometryBase, time: RosTime) -> None:
        if self.publisher_background is None:
            return
        if not isinstance(background, PointCloud | TensorPointCloud):
            self.get_logger().error("ROS publishing of other geometries than PointClouds not implemented")
            return
        structured_array = np.zeros(
            len(background),
        ...
        points = background.numpy_points()
        obj_colors = (background.numpy_colors() * 255).astype(np.uint32)
        structured_array["x"] = points[:, 0]
        structured_array["y"] = points[:, 1]
        structured_array["z"] = points[:, 2]
        structured_array["r"] = obj_colors[:, 0]
        structured_array["g"] = obj_colors[:, 1]
        structured_array["b"] = obj_colors[:, 2]
        structured_array = merge_rgb_fields(structured_array)
        msg = array_to_pointcloud2(structured_array, frame_id=self.global_frame, stamp=RosTime.to_msg(time))
    ...
    def _publish_exploration_map(self, exploration_grid: OccupancyGrid, time: RosTime) -> None:
    ...
    def run(self) -> None:
        ...
        while rclpy.ok():
            ...
            if below_max_rate(now, last_update_own_clock) and self.input is not None and self._try_complete_input():
                last_update_own_clock = now
                last_update_msg_clock = self.input[2]
                set_outdated()
            ...
            if (
                self.publishing_rate_background_pointcloud != 0
                and published_background_outdated
                and (
                    self.publishing_rate_background_pointcloud == -1
                    or (now - last_background_published_time).nanoseconds / 1e9
                    > 1 / self.publishing_rate_background_pointcloud
                )
            ):
                self._publish_background_pointcloud(self.scene.get_background(), time=now)
